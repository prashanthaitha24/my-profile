resource "aws_iam_role" "eks" { name="${var.name}-cluster-role"
...
resource "aws_iam_role_policy_attachment" "eks_cluster" { role=aws_iam_role.eks.name, policy_arn="arn:aws:iam::aws:policy/AmazonEKSClusterPolicy" }
resource "aws_eks_cluster" "this" {
  name=var.name, role_arn=aws_iam_role.eks.arn, version="1.29"
  vpc_config { subnet_ids=var.subnet_ids }
  depends_on = [aws_iam_role_policy_attachment.eks_cluster]
  tags=var.tags
}
resource "aws_iam_role" "node" { name="${var.name}-node-role"
